...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        databaseAdReference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                snapshot.children.reversed().forEach {
                    if (snapshot.child("${it.key}").child("img").value != null) {
                        adImagesPath.add(snapshot.child("${it.key}").child("img").value as String)

                    }
                    if (snapshot.child("${it.key}").child("id").value != null) {
                        adIds.add(snapshot.child("${it.key}").child("id").value as String)
                    } else {
                        adIds.add("0")
                    }
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(context, "Error: can't load the images path", Toast.LENGTH_SHORT)
                    .show()
            }
        })
        ...
        databaseTimelineReference.addValueEventListener(object : ValueEventListener {
                                ...
                                override fun onDataChange(snapshot: DataSnapshot) {
                                    try {
                                        snapshot.children.forEach {
                                            ...
                                            Picasso.get().load(
                                                snapshot.child("${it.key}")
                                                    .child("img").value as String
                                            ).into(imageView)
                                                            ...
                                                            override fun onDataChange(snapshot: DataSnapshot) {
                                                                try {
                                                                    FirebaseStorage.getInstance()
                                                                        .getReferenceFromUrl(
                                                                            snapshot.child("p_1/img").value.toString()
                                                                        ).delete()
                                                                    FirebaseStorage.getInstance()
                                                                        .getReferenceFromUrl(
                                                                            snapshot.child("p_2/img").value.toString()
                                                                        ).delete()
                                                                } catch (e: Exception) {
                                                                    e.printStackTrace()
                                                                }
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(context, "Error: can't the timeline images", Toast.LENGTH_SHORT)
                    .show()
            }
        ...
        if (CheckAdmin.isAdmin(FirebaseAuth.getInstance().currentUser)) {
    ...
    private fun setViewPager(rootView: View) {
        ...
        val adapter: PagerAdapter =
            HomeViewPagerAdapter(this@HomeFragment, adImagesPath, adIds, context)
